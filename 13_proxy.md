...
**BEST PRACTICE**: Prevent multiple invocations of such initializer functions using the `initializer` modifier from OpenZeppelin’s `Initializable` library
...
If the logic implementation calls selfdestruct, its code is destroyed.

All proxy contract instances will end up delegating calls to an address without any code.

**BEST PRATICE**: Entirely avoid selfdestruct and delegatecall in proxy-based contracts
